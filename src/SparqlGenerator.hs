...
--     mrsVar <- var
--     triple mrsVar (rdf .:. "type") (mrs .:. "MRS")
--     s <- middleTransform pred $ return $ TransformData (return Map.empty) mrsVar prefixes []
--     --patterns s
--     selectVars [mrsVar]
...
--   do
--     os <- s
--     s1 <- middleTransform pred1 $ return $ TransformData (varDict os) (mrsVar os) (prefixes os) []
--     s2 <- middleTransform pred2 $ return $ TransformData (varDict s1) (mrsVar os) (prefixes os) []
--     return $ TransformData (varDict s2) (mrsVar os) (prefixes os) (patterns os ++ patterns s1 ++ patterns s2)
...
--     s2 <- middleTransform pred2 $ return $ TransformData (varDict s1) (mrsVar os) (prefixes os) []
--     t1 <- union
--           (fmap patterns $ middleTransform pred1 $ return $ TransformData (varDict os) (mrsVar os) (prefixes os) [])
--           (fmap patterns $  middleTransform pred2 $ return $ TransformData (varDict s2) (mrsVar os) (prefixes os) []) -- p1 :: Pattern
--     return $ TransformData (varDict s2) (mrsVar os) (prefixes os) (patterns os ++ [t1])
    
-- -- Need a fix/verification:
-- middleTransform (Not pred) s =
--   do
--     os <- s
--     s1 <- middleTransform pred $ return $ TransformData (varDict os) (mrsVar os) (prefixes os) []
--     t1 <- filterNotExists $ return $ patterns s1
--     return $ TransformData (varDict s1) (mrsVar os) (prefixes os) (patterns os ++ [t1])
...
--     --       (triple (mrsVar s1) (head (prefixes s1) .:. "hasEP") epVar)
--     --       (return s1)
--     triple_ (mrsVar s1) (head (prefixes s1) .:. "hasEP") epVar
--     s3 <- putTop pred epVar (return s1)
--     --Falta processar predicado direito; considerando predmod e regex.
--     s4 <- putPred pred epVar (return s3)
--     s5 <- processArgs argList epVar (return s4)
--     return s5
...
--     --s3 <- putTop pred epVar (return s2)
--     --Falta processar predicado direito; considerando predmod e regex.
--     s4 <- putPred pred epVar (return s2)
--     s5 <- processArgs argList epVar (return s4)
--     return s5
...
--     pat <- t
--     os <- s
--     return $ TransformData (varDict os) (mrsVar os) (prefixes os) (patterns os ++ [pat]) 
