...
--   do
--     notPattern <- filterNotExists $ middleTransform pred mrsVar dict
--     return [notPattern]
...
-- createSelectQuery . transformation . predx . fst . last $ readP_to_S WQL.wql "x:_run*[ARG1 y, ARG2 y]"
-- createSelectQuery . transformation . predx . fst . last $ readP_to_S WQL.wql "x:_run*[ARG1 y, ARG2 y] | z:_be*"
-- createSelectQuery . transformation . predx . fst . last $ readP_to_S WQL.wql "x:_run*[ARG1 y, ARG2 y] y:_ah*"
-- createSelectQuery . transformation . predx . fst . last $ readP_to_S WQL.wql "x:_run*[ARG1 y, ARG2 y] y:_ah* | z:_be*"
-- createSelectQuery . transformation . predx . fst . last $ readP_to_S WQL.wql "x:_run*[ARG1 y, ARG2 y] y:_ah* | !(z:_be*)" : não funciona; testar notExists pra entender
